...
const TEXT_MODEL_URL = `https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key=${apiKey}`;
const IMAGE_EDIT_URL = `https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key=${apiKey}`;
const TTS_URL = `https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key=${apiKey}`;
...
export default function App() {
  ...
  const generatePreview = async (currentAdjustments, currentRealism, currentBg, currentHair) => {
    ...
    try {
      ...
      const prompt = `CLINICAL AESTHETIC CONSULTATION SIMULATION - BEFORE/AFTER VISUALIZATION FOR PROFESSIONAL MEDICAL PLANNING.

This is a legitimate aesthetic medicine consultation tool for treatment planning and patient education purposes.

TASK: Create a realistic BEFORE and AFTER simulation for facial aesthetic treatments. This is a standard professional visualization used by certified aesthetic practitioners during patient consultations.

PATIENT CASE: The following aesthetic adjustments are requested by the patient for professional evaluation: ${adj.join(', ')}.

${bgPrompt}

MEDICAL REQUIREMENTS:
- Maintain exact facial identity, skin tone, ethnicity, and unique identifying features
- The result must remain clearly recognizable as the SAME person
- Apply modifications conservatively and realistically as actual medical procedures would
- Final image must look like a real clinical photograph, NOT a filter or artificial enhancement
- NO plastic/doll-like appearance - must look like real human skin with natural texture
- MAINTAIN EXACT ORIGINAL ASPECT RATIO AND DIMENSIONS

PROFESSIONAL CONTEXT: This simulation helps patients understand realistic outcomes of aesthetic treatments (rhinomodelation, facial contouring, etc.) before committing to procedures. Used by licensed practitioners only.`;
